...
export default function TeachingMode({ folder, files, links, accent, t, onClose, startWithPlanner = false, initialSessionId = null }) {
  ...
  useEffect(() => {
    const onKey = (event) => {
      ...
      if (!typing && !event.metaKey && !event.ctrlKey && !event.altKey && event.key.toLowerCase() === 'l') {
        ...
        setLaserActive((value) => { if (value) setLaserPoint(null); return !value; });
      }
      if (event.key === 'Escape') { if (timerFullscreen) setTimerFullscreen(false); else onClose?.(); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [onClose, timerFullscreen]);

  const updateLaserPoint = (event) => {
    if (laserActive) setLaserPoint({ x: event.clientX, y: event.clientY });
  };
  const hideLaserForTouch = (event) => {
    if (event.pointerType !== 'mouse') setLaserPoint(null);
  };
  ...
  return (
    <div className="lm-teaching-mode" onPointerMove={updateLaserPoint} onPointerDown={updateLaserPoint} onPointerUp={hideLaserForTouch} onPointerCancel={hideLaserForTouch} onPointerLeave={() => setLaserPoint(null)} style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'var(--c-bg)', color: 'var(--c-text)', display: 'grid', gridTemplateColumns: 'minmax(280px, 360px) minmax(0, 1fr)' }}>
      <aside className={`lm-teaching-sidebar lm-teaching-command-panel${mobilePhaseFocus ? ' is-hidden' : ''}`} style={{ borderRight: '1px solid var(--c-border)', background: 'var(--c-surface)', padding: 18, overflow: 'auto' }}>
        <div className="lm-teaching-lesson-head" style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 18 }}><div><div style={{ fontSize: 10, fontWeight: 800, letterSpacing: .8, textTransform: 'uppercase', color: accent }}>Unterricht läuft</div><h2 style={{ margin: '4px 0 0', fontSize: 20 }}>{folder.name}</h2></div><button aria-label="Unterrichtsmodus schließen" onClick={onClose} style={{ width: 30, height: 30, border: '1px solid var(--c-border)', borderRadius: 8, background: 'transparent', color: 'var(--c-text-2)', cursor: 'pointer' }}>×</button></div>
      ...
      {laserActive && laserPoint && <div className="lm-teaching-laser" aria-hidden="true" style={{ left: laserPoint.x, top: laserPoint.y }} />}
    </div>
  );
}
