  ...
  return (
    <div className="lm-teaching-mode" onPointerMove={updateLaserPoint} onPointerDown={updateLaserPoint} onPointerUp={hideLaserForTouch} onPointerCancel={hideLaserForTouch} onPointerLeave={() => setLaserPoint(null)} style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'var(--c-bg)', color: 'var(--c-text)', display: 'grid', gridTemplateColumns: 'minmax(280px, 360px) minmax(0, 1fr)' }}>
      <aside className={`lm-teaching-sidebar${mobilePhaseFocus ? ' is-hidden' : ''}`} style={{ borderRight: '1px solid var(--c-border)', background: 'var(--c-surface)', padding: 18, overflow: 'auto' }}>
        ...
        <button className={`lm-laser-toggle${laserActive ? ' is-active' : ''}`} aria-pressed={laserActive} title="Laserpointer umschalten (L)" onClick={() => { setLaserActive((value) => !value); setLaserPoint(null); }}>Laserpointer <kbd>L</kbd></button>
      </aside>
